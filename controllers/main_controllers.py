...
class BookController:
    ...
    def create_book_with_authors_and_categories(self, db: Session, book_data: CreateBook, author_names: List[str],
        ...
        try:
            ...
            new_book = Book(
                **book_data.dict())  # Производиться распоковка словоря и передача всех данных и переменных в Book

            # Связываем книгу с авторами и категориями
            new_book.authors.extend(authors)  # В отличие от append принимает кортежи, словари и списки
    ...
    async def count_books_by_category(self, db: AsyncSession):
        async with db as session:
            # Выполняем асинхронный запрос, который загружает категории и соответствующие книги
            query = select(Categories.category_name, func.count(Book.id).label('book_count')). \
            ...
            book_counts_by_category = [{'category_name': category_name, 'book_count': book_count} for
                                       category_name, book_count in category_counts]

            return book_counts_by_category

    async def get_books(self, db: AsyncSession, skip: int = 0, limit: int = 100):
        async with db as session:
    ...
    async def search_books_by_title(self, db: AsyncSession, query: str):
        async with db.begin():
            result = await db.execute(select(Book).filter(Book.title.ilike(f'%{query}%')))
            books = result.scalars().all()
            return books

    async def get_books_by_category(self, db: AsyncSession, category_name: str, skip: int = 0, limit: int = 10):
        async with db as session:
            # Поиск категории по имени
            category = await session.execute(select(Categories).filter(Categories.category_name == category_name))
            ...
            books_with_category = await session.execute(
                select(Book).join(Book.categories).filter(Categories.category_id == category.category_id).offset(
                    skip).limit(limit)
            )
            books_with_category = books_with_category.scalars().all()
...
class Author_Book_Controller:
    ...
    def delete_author_and_associated_books(self, db: Session, author_id: int):
        ...
        if author:
            # Удалите связи автора с книгами
    ...
    async def get_author_and_books(self, db: AsyncSession, author_name: str):
        async with db as session:
            author = await session.execute(select(Author).filter(Author.author_name == author_name))
            author = author.scalar()
            if author:
                author_id = author.author_id
                books = await session.execute(
                    select(Book).join(Book.authors).filter(Author.author_id == author_id)
                )
                books = books.scalars().all()
                return author, books
        return None, []

    async def update_author(self, db: AsyncSession, author_id: int, request: UpdateAuthorRequest):
        async with db as session:
            author = await session.execute(select(Author).filter(Author.author_id == author_id))
...
class Category_controller:
    ...
    async def update_category(self, db: AsyncSession, category_id: int, request: UpdateCategoryRequest):
        async with db as session:
            category = await session.execute(select(Categories).filter(Categories.category_id == category_id))
...
#
#     def create_user(self, db: Session, user_data: Create_User):
#         user = User(username=user_data.username, name=user_data.name, surname=user_data.surname, email=user_data.email,
#                     user_password=user_data.password, role_id=user_data.role)
#
#         db.add(user)
#         db.commit()
#         db.refresh(user)
#         return user
#
#     def create_role(self, db: Session, role_name: str):
#         role = Role_User(name=role_name)
#         db.add(role)
#         db.commit()
#         db.refresh(role)
#         return role
